...
#"================================================================"
##### start for loop#####
for(i in 1:length(ordered_gpkg)){
  skip_to_next <- FALSE
  tryCatch({
    print("================================================================")
    print("================================================================")
    ...
    if(!any(spp_summary_birds_mammals$id_no == taxon_id)){
      print(paste0("Skipping taxon id ", taxon_id, " because excluded a priori from the calculations"))
      skip_to_next <<- TRUE
      if(skip_to_next) {next}
    }
    if(!any(spp_habitat_birds_mammals$id_no == taxon_id)){
      print(paste0("Skipping taxon id ", taxon_id, " because no habitat preferences are available."))
      skip_to_next <<- TRUE
      if(skip_to_next) {next}
    }
    ...
    if(!model_validation && (file.exists(file_bred) & file.exists(file_nonb))){
      print(paste0("Skipping taxon id ", taxon_id, " with index ", i, "; species already processed."))
      skip_to_next <<- TRUE;
    ...
    if(skip_to_next) { next }
    print(paste0("Processing taxon id ", taxon_id, " with index ", i))
  ...
  error = function(e) { 
    print(paste0("Error is ", e)); 
    print(paste0("Error with ", spp_summary_data$binomial, "; taxon id = ", taxon_id, "; processed with index = ", i));
    if(type != "LR_validation_habitat_only") print(paste0("The habitat codes are: ", unique(spp_habitat_data$code)));
    if(str_detect(e, "argument to x does not contain any terrestrial species")){
      check_errors[[paste0(taxon_id)]] <- list(error = e,
                                               nrow_range = nrow(spp_range_data),
                                               species = spp_summary_data$binomial,
                                               habitat_codes = unique(spp_habitat_data$code),
    ...
    msg <- paste0("Cannot process taxon id ", taxon_id, " | ", unique(spp_summary_data$binomial)," with index ", i,
                  " because of an error. Have a look at the check_errors", tmp_name, ".RData file.")
    if(is.null(not_processed_species[[paste0(taxon_id)]])){
      not_processed_species[[paste0(taxon_id)]] <- msg
    } else {
      tmp <- not_processed_species[[paste0(taxon_id)]]
    ...
    save(not_processed_species, file = paste0(output_errors, "not_processed_species", tmp_name, ".RData"))
    save(check_errors, file = paste0(output_errors, "check_errors", tmp_name, ".RData"))
    skip_to_next <<- TRUE;
    })
